...
import { useCreatePurchaseRequisition } from '../hooks/useQueries';
...
import type { PurchaseRequisitionItem } from '../backend';

interface CreatePurchaseRequisitionFormProps {
  onSuccess: () => void;
}
...
export default function CreatePurchaseRequisitionForm({ onSuccess }: CreatePurchaseRequisitionFormProps) {
  const { register, handleSubmit, control, formState: { errors }, setValue } = useForm<PurchaseRequisitionFormData>({
    defaultValues: {
      requestedBy: '',
      department: '',
    ...
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
      <div className="grid grid-cols-2 gap-4">
        <div className="space-y-2">
          ...
          />
          {errors.requestedBy && (
            <p className="text-sm text-destructive">{errors.requestedBy.message}</p>
          )}
        </div>
